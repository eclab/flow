...
\noindent {\it Tip\quad} Occasionally you may find that you need a module which allows you to control several modulations at once by dragging a single dial.  You can easily do this in Mod Math: for example, you could choose \(\boldsymbol{+}\) , set Signal B to 0, hook up the output modulation to your various input modulations to control, and drag the Signal A dial.\\
\\
...
\paragraph{The Current Patch Format}  The format is in JSON and is as follows:

\begin{itemize}
\item {\tt \{  "flow":{\it file-format-version},\quad "name":{\it patch-name},\quad "modules":{\it modules} \} }
\item {\it file-format-version:}\quad an integer \(\geq 0\) representing the version of this file format.  Presently 0. 
\item {\it patch-name:}\quad a string. 
\item {\it modules:}\quad {\tt [ {\it module,*} ] }
\item {\it module:}\quad {\tt \{ "v":{\it module-version}\quad "class":{\it module-classname}\quad "id":{\it module-id}\quad "opt":{\it options}\\\hspace{\fill}\quad "unit":{\it unit-inputs}\quad "mod":{\it modulation-inputs} \quad{\it optional-data} \}}
...
\item {\it optional-data:}\quad A JSON object in the form {\tt \{...\}} which the module may use as it likes to store additional data.  This object does not need to appear in the patch at all.  See Wavetable and Draw for examples of usage of optional data.
