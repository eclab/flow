...
\paragraph{Mix} This is a straightforward mixer: the amplitudes of the partials of four different inputs can be mixed, with their respective gains.  The {\it frequencies} of the partials of the first input are used for the frequencies of the final partials.  In most cases, it's likely that all four inputs have partials with the same frequency, so it wouldn't matter.  Be sure to read ``Mixing Strategies'', Section~\ref{mixingstrategies}.  See also {\bf Amp Math} and {\bf Combine} and {\bf Fill} and {\bf Morph.}
...
There are four special modules: {\bf In}, {\bf Out}, {\bf Options}, and {\bf Macro}.  The {\bf Out} module is critical in every patch.  {\bf In} and {\bf Macro} enable {\it macros}\.---\,and {\bf Out} helps out here as well.

\paragraph{Options}  The Options module is an odd duck: it doesn't produce any sounds or modulation at all.  Instead, it enables you to change a module's checkboxes and comboboxes using a modulation signal.  To do this, note that Options has a modulation input and a sound input port both called {\bf Target}.   Plug one of your target module's modulation or sound outputs into the appropriate Target port: this tells Options what target module it should be modifying.  Don't attach to both of them, or Options will just use the sound input port.
