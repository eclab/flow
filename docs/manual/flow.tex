...
\caption{Envelope modulation curves, shown both in rising and falling scenarios.  Solid lines are (right to left) {\it Linear}, \(x^2\), \(x^4\), \(x^8\), \(x^{16}\), and \(x^{32}\).  Dashed lines are (right to left) \((x^2 + x^8)/2\),  \((x^4 + x^{16})/2\), and  \((x^8 + x^{32})/2\).  The dotted line is {\it Step}, which stays put until the very end, at which time it suddenly jumps to the ultimate value. }
\label{curves}
\end{wrapfigure}
