...
\paragraph{Changing Patch Order}
...
If you drag the titlebar of a subpatch clear into the region of the {\it primary patch}, it will {\it swap} the subpatch with the primary patch.  Flow will try its best to swap the allocated number of voices and the MIDI channel of the primary and subpatches as well; and reverb (which is global) will be preserved from the previous primary patch.  MIDI has some caveats however.  First, only the Primary patch can perform MPE: so if you are swapping a primary patch that uses MPE, when it becomes a subpatch its MIDI channel will become ``No Channel''.  Similarly, only subpatches can have ``No Channel'' as a MIDI channel option,\footnote{This may change in the future.} so if one of them is swapped to a primary path, its MIDI channel will change to ``All Channels''.

\paragraph{Loading and Clearing the Primary Patch}  Obviously you could load a new primary patch or clear it by choosing {\it Load Patch...} or {\it New Patch}: but this will clear out the subpatches as well.  There are times when you'd just like to replace the primary patch but keep the existing subpatches around.
...
Here's a common scenario.  You have recorded MIDI for several tracks in your DAW, and you're currently recording MIDI for a new track while playing the old ones.  This new track is connected to Flow's Primary Patch, while the several old tracks are connected to subpatches.  When you have finished recording the new track, you load a new patch into Flow's Primary Patch, demoting the current Primary Patch to a subpatch, and you begin anew.

To do this, you'd assign each subpatch a unique MIDI channel.  The Primary patch also gets its own MIDI channel.  Your keyboard is connected to the DAW's recording track, and the DAW is rerouting it to the primary patch's MIDI.  Flow's sound output is connected to the DAW in anticipation of eventually recording the audio: for now the DAW is just routing it to the speakers.

\paragraph{Latency} There is a big problem with this scenaro: very high latency.  Your MIDI keyboard is routed to the DAW, which is then {\it rerouting} it to Flow, which is then sending audio to the DAW, which is then {\it rerouting} the audio to your speakers.
...
\item Keep in mind that some patches are much more costly per-voice than others.
\end{itemize}

\paragraph{Minimizing Tracks} I am cheap and don't own a full-fledge copy of Abelton: I just have Ableton Live Lite.  The problem with this is that I am restricted to 8 tracks total, including the audio track (I record multiple MIDI tracks, then play them together with subpatches and record a final audio track).  This means that I can have at most 7 MIDI channels going into Flow.  How then can I play (say) 12 different sounds?
...
\paragraph{Gain} At the end of the day I can get my whole song to play without glitching, but it still clips here and there.  Rather than go through and adjust the gain of each subpatch separately, I just pull down the master gain (see the {\bf MIDI and Audio Options} window).     Note that Master Gain is not saved with patches and is reset when you restart Flow.
