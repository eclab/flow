...
{\name} is very computationally expensive and benefits from some tuning.  If you select {\bf Tuning Parameters} from the {\bf Play} menu, you can at present change three parameters.  Pressing {\it Okay} or {\it Reset} (which resets to defaults) will change the parameters for the {\it next} time you run {\name}, not the current session. 
...
\item {\bf Audio Buffer Size} (default: 2048)\qquad This is the size of the audio buffer that {\name} keeps filled for your operating system to tap into to generate sounds.  Some operating systems require a large buffer size: for example, OS X technically can run with a buffer of only 1024, but on my laptop realistically {\name} name requires a buffer larger than that (1536 or 2048 seem to work).  Large buffers incur significantly higher latency, which you don't want. On Linux you can get away with much smaller buffers: see the Linux portion of Section~\ref{audioloopback} for more information.  I can't help you much regarding Windows.
\item {\bf Partials} (64--256, default: 128)\qquad This is the number of partials passed around from module to module.  More partials will produce a richer sound, particularly in bass notes, but incur a much higher performance penalty.
}
\end{itemize}
...
\item {\tt NUM_OUTPUTS_PER_THRES}\qquad Affects the number of output threads generated to convert the partial results into the sound.  Fewer threads, and you've got more work being done by a single processor (which may not be able to take the load); but more threads means more total work given the thread context switching.  By default this is 2, which seems to work well on my laptop.
...
\item {\it file-format-version:}\quad an integer \(\geq 0\) indicating the version of Flow which created this file.  Presently 0. 
