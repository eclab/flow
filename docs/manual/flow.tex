...
Notice we didn't mention {\it phase}.  Phase is the horizontal shift in the sine wave, as shown in Figure \ref{phases}.  As it turns out, humans can't easily distinguish between sounds whose partials only differ in phase (though they can tell when phase is rapidly {\it changing}).  We just don't have the hardware built-in to do it.  This means that we can ignore the phase component of our sine waves in order to simplify the synthesis process: we'll assume that all sine waves have a phase angle of \(0\degree\).  Indeed, a great many additive synthesizers, including {\name}, don't consider phase in order to keep things simpler (and faster).
...
\begin{center}
\begin{tabular}{rl}
Number of Partials&Always 128 (without phase)\\
...
MIDI Features&MPE, Clock, Channel and Polyphonic Aftertouch, Bend, NRPN, RPN, CC\\
\end{tabular}
\end{center}
...
After you fire up {\name}, you're presented with a near-empty {\bf rack} and can start adding {\bf modules} to it.  Then you tweak the parameters of modules and wire them up by dragging from output jacks to input jacks or to dials.  The result might look something like the example patch in Figure~\ref{screenshot}.
...
\subsection{Adding and Moving Modules}  Modules are added to the rack by choosing them from the {\bf Modules} menu.  You can move a module to another location by dragging its title bar.  You can delete a module by clicking on its close box: the little white dot to the left of its title.
...
Modules can send any number of partials and modulation signals, and they can receive any number of the same.  But some modules are intended {\it primarily} to {\bf create modulation signals} (their title bars are red), to {\bf modify incoming modulation signals and emit new versions} (light red title bars), to {\bf create partials} (blue), or to {\bf modify incoming modulation signals and emit new versions} (light blue).  A few system-level modules, such as the {\bf Out} module, have black title bars.  Macros (see Section~\ref{aboutmacros}) have green title bars.
...
So how do you play a using keyboard?  You attach it to {\name} by selecting {\bf MIDI and Audio Preferences} in the {\bf Play} menu, then choosing the appropriate MIDI channel and MIDI device.  Similarly you can choose the appropriate output audio device for your computer, if you have multiple ones.
...
\item {\bf On Linux}\quad I can't help you here either.
...
\item \(\boldsymbol -\)\quad Does \(\max(A - B,\ 0)\)
...
\paragraph{Draw}   Lets you draw partials, which are then output,   You can constrain the partials being drawn to a variety of subsets.  You can also save your scribbles and load them again, clear them, maximize, normalize, and standardize them, and (importantly) take a snapshot of partials from an incoming source.  The module can be collapsed to a smaller size if you wish.
}
...
\paragraph{Harmonics}  Outputs the first 16 harmonics, whose amplitude you can define independently.  See also {\bf Partials}.
...
\paragraph{Rectified}  The harmonics of a rectified wave, which is essentially \(|\sin(x)|\).

\paragraph{Sawtooth}  The harmonics of a sawtooth wave.

\paragraph{Sine}  A single harmonic at 1.0 amplitude.  The harmonic any from 0 to 127.
...
%\paragraph{Wave}  A wave from the set of Microwave II/XT waves.  You have the additional option of {\it Normalizing} the partials. Note that Waldorf asked us to not include this data; and although they likely don't hold IP over these, we have elected to do as they wished.  We are looking for public domain wavetables we might use instead. 

%\paragraph{Wavetable}  A wavetable from the set of Microwave II/XT waves with a modulatable position.  Note that Waldorf asked us to not include this data; and although they likely don't hold IP over these, we have elected to do as they wished.  We are looking for public domain wavetables we might use instead.

\paragraph{Wavetable}  Lets you load wavetables (as ``.WAV'' files) from https:/\!/waveeditonline.com/, then wander through them using the provided modulation.
}
...
\vspace{-1em}{\setlength\fboxsep{0in}\framebox{\includegraphics[width=1.2in]{constrain.png}}}
\vspace{-2em}
...
\paragraph{Fatten}  The top half partials are discarded and reused to create detuned versions of the lower half partials.  The detuning amount is specified.
...
\paragraph{Shift} This shifts all the partials in your input in a certain way.  {\it Shift} specifies by how much (this is easily modulated), and {\it Bound} scales the shifting.  There are three ways you can shift.  First you can shift the {\it Pitch}: this is essentially multiplying the partials by a certain amount.  Second, you can shift the {\it Frequency}: this {\it adds} a certain amount to the partials, which creates an inharmonic shift effect.  Third, you can shift the {\it Partials}: this moves each partial towards the former frequency of the partial above it (or below it).
...
\paragraph{Partial Filter} This is a filter based on the partials of an incoming signal.  The partials form nodes in the filter, and the filter function interpolates between them.  Try drawing partials with {\bf Draw} and feeding them into Partial Filter to filter some sound to get the idea.  The partials in question can be interpreted as fixed frequencies or as frequencies in the same scale as the incoming sound (that is, they're scaled by the current pitch just like the input is).  If they're interpreted as fixed frequencies, then a partial at position 1.0 (where the fundamental normally is), or indeed \(\leq 1.0\), would represent 0Hz, a partial at 2.0 would represent 100Hz, a partial at 3.0 would represent 200Hz, and so on.  Typical standardized partials (going from 1 to 128) would thus cover 0Hz to 12.7KHz.  That may not be far enough for you; but you can easily {\it double} this scale, so 1.0 is 0Hz, 2.0 is 200Hz, etc., clear up to 128.0 being 25.4KHz.\footnote{You could have stretched this with {\bf Scale} or {\bf Shift} too of course.}
...
%\vspace{-2em}
%\includegraphics[width=1.6in]{Other}
...
When you save a patch, you're asked to give it a name.  You might ask: why?  The main reason for this is that a Macro module doesn't say ``Macro'' in its title bar.  Instead it'll display the patch name you provided.

You can also rename the input and output partials and modulation ports of your Macro.  This is accomplished by renaming their compatriots in your In and Out modules.  Note that, unlike in other modules, the ports in your In and Out modules have {\bf underlined labels}.  Just click on a label and you'll be asked to give it a new name.
...
\paragraph{Performance tuning parameters} You should be aware of several tunable parameters in Output.java which impact on the performance of these threads and ultimately the performance of the application.  Some of these are constants, others are variables which Output.java loads from Java preferences in its constructor (and it'd be best if you changed them there).
...
\begin{itemize}
\item {\tt NUM\_PARTIALS}\qquad is the number of partials being handed from module to module.  This is by default 128.  You can change this value (for example, reduce it), though it might impact on how patches work as some modules make assumptions about the number of partials. 
\end{itemize}
...
public class MyModulation extends Modulation
    ...
    public void reset()
        {
        super.reset();
        firstTick = sound.getOutput().getTick();
        }
...
The method {\tt sound.getOutput.getTick()} gives us the current {\it tick}.  This will be an integer value representing the number of samples that have been generated so far.\footnote{{\it getTick()} is currently a integer for threading efficiency reasons.  But this means it overflows after about 13 hours. We may change it to a long later.}  Here we're using it to switch from 0.0 to 1.0 every second.  Note the use of division: this is inefficient and would be better handled using a countdown of some sort, but for simplicity's sake here...
...
Now let's try it out.  Add the {\tt flow.modules.MyModulation.class} class to the list of modules called {\tt static final Class[] modules} in 
 {\tt additive/gui/AppMenu.java}.  Now create an empty patch, add an {\bf Out} and a {\bf MyModulation} to it, and hook up our {\it out} and {\it other} ports to Out's {\it 1} and {\it 2} dials.  Check ``free'', and you should start seeing stuff on the oscilloscopes.  Change the rate and see what happens.
...
\paragraph{Test the Code}  Try it out again!  And that concludes our little tutorial.
