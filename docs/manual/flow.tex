...
To make sound, Flow runs two procedures in parallel.  The {\it voice facility} repeatedly gets the latest MIDI data, pumps each of the modules in each of the voices, and produces and posts the final set of partials.  Simultaneously the {\it output facility} repeatedly grabs the most recently posted partials and generates some \(N\) mono or stereo samples from them. 

If the output facility hasn't grabbed the most recently posted partials, and the voice facility has even newer partials to give it, it will wait until the output facility grabs the older ones before posting the newer ones.  This means that if \(N\) is {\it high}, then the voice facility will wait for longer times.  This causes two bad thing.  First, it means that the voice facility will take longer to respond to new MIDI messages, resulting in a noticeable latency.  Second, it means that sounds will change partials more slowly: you may or may not notice this second effect (though if \(N=1\), the partials update is so fast that you can effectively do FM with the LFOs).  So why not keep this value low?  Because low values of \(N\) mean that the output facility will constantly be grabbing the latest partials, which incurs a significant computational load.  
...
\paragraph{Switch} This module switches among four different inputs.  You select an input by sending a trigger to its corresponding dial.  Switch will then interpolate the amplitudes and frequencies between the old input and the new one until the new one has completely taken over: the rate of interpolation is given by {\it Rate}.  The purpose of non-zero Rate values is to prevent popping from sudden changes.

Each input also has a corresponding modulation output: these work just like the ones in {\bf Morph}: when a sound is selected, its output will generate a trigger; and modulations also output their current interpolation value.  Initially, Switch starts at Input A.  If {\it Free} is set, then Switch doesn't reset to Input A every time you press a note\,---\,it keeps morphing from wherever it left off.  

Switch is useful for playing sequences of sounds: you can do this by attaching an envelope of some sort to it, and routing each of its individual stage triggers into Switch's trigger inputs.   When Switch switches to a new sound in the sequence, you probably will want to reset the sound as if it had been just played with Note On.  This is most easily done by feeding Switch's trigger output into the ``On Tr'' input of the sound's envelopes or LFOs; or you could just save the sound as a patch, then load it as a Macro and feed the trigger output into the ``On Tr'' input of the macro.   See also {\bf Morph}.
