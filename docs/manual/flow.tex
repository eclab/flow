...
\subsection{Adding, Moving, Copying, and Closing Modules}  Modules are added to the rack by choosing them from one of the menus: {\bf Modulation}, {\bf Sources}, {\bf Shapers}, and {\bf Other}.  You can move a module to another location by dragging its title bar.  You can also copy a module to another location by dragging its title bar while holding the Control key (on the Mac, you can also hold the Option key).  Finally, you can delete a module by clicking on its close box: the little white dot to the left of its title.
...
\paragraph{Joystick}  A controller that lets you simultaneously change four different modulation values.   Click {\bf Show} to reveal the Joystick.  Each value is associated with a corner (\(\nwarrow \nearrow \swarrow \searrow\)) and is highest in that position.\footnote{Technically the value of a modulation is \(1.0 - w\), where \(w\) is the maximum of the \(x\) and \(y\) distances from the corner, assuming that the joystick has a width and height of 1.0.}   The center location is a uniform mixture of all four modulations.  You can click on a location to move the joystick there, or you can drag the joystick: it turns red while dragged.  All voices are simultaneously affected when you manually drag the joystick.

True to vector synthesis tradition, you can also {\it record} a drag sequence, and once you are finished, the recording will be played back each time you press a key, affecting that voice.  You record a sequence by clicking on the {\bf Record} button, which arms it.  The joystick turns green and the button now says {\bf Cancel}, and clicking on it again will (of course) cancel the recording.  Recording begins as soon as you grab the joystick and start moving it (the button will then say {\bf Recording} and the joystick will turn red as usual).  When you let go, the joystick turns blue again, and the button now says {\bf Clear}, indicating that you have made a recording.  Playing notes will now trigger the recorded movements (independent of one another).  If you now click the button, you will erase the recording and go back to {\bf Record}.   You can always click the little triangle to hide the joystick: the recording remains (and is saved with your patch).
...
\paragraph{Partials Visualization}  When you connect to the {\bf A} port, the resulting partials are also displayed on the left partials display in your rack.  The {\bf B} (Aux) port is also useful for displaying partials (it shows them on the right partials display).  
