...
Polyphony&1--16, user-selectable.\\
Multitimbrality&Up to 16 unique sounds, each on its own MIDI channel.  Compatible with MPE.\\
...
\vspace{-2.2em}{\setlength\fboxsep{0in}\framebox{\includegraphics[width=1.25in]{constrain.png}}}
\vspace{-2em}
\caption{Constraints}
\vspace{-1em}
...
\subsection{Multitimbral Patches}
...
In most synthesizers, a multitimbral patch is different from a single patch.  You'd make a bunch of single patches, then make one multitimbral patch which combines them.  Flow's multitmbral patch model is different.  In Flow, you load a single patch, then you attach one or more additional single patches to it as {\bf subpatches}.  The original patch is known as the {\bf primary patch}.  You can save this combined patch out to a standard file and load it again.

Subpatches are added to the primary patch by choosing {\it Load Subpatch...} in the {\it File} menu.  You can remove a subpatch by clicking on its close box, just like removing a module.  You can also drag subpatches to reorder them relative to one another, just like modules.

Only the primary patch can be edited: the subpatches will appear as thin rows beneath its modules.  For each subpatch you can make the following settings:
...
You can see the number of remaining voices for the primary patch by looking at its {\bf Out} module.  If the title just says {\it Out}, then it has full voices.  Otherwise it'll say something like {\it Out (13 Voices)}.

One very confusing situation arises when you have selected {\it Monophonic} from the {\it Play} menu.  Here Flow only uses a single voice: that voice must go to the primary patch, so the subpatches all have zero voices allocated.  To make this situation clear, the subpatches will show {\bf [M]} (for ``monophonic'') for the number of voices allocated, and the word "(Mono") will appear in the {\bf Out} module of the primary patch.
...
Subpatches cannot use MPE, nor can the be assigned to ``Any Channel''.  Only the primary patch can.

\item {\bf Gain}\quad This sets the overall volume of the subpatch in the final mix.  This is exactly the same as the {\it Gain} knob on the {\bf Out} module of the primary patch, discussed later in Section~\ref{outmodule}.  And in fact, the Gain knob can be modulated: if your subpatch was originally modulating its Gain knob before you loaded it as a subpatch, it will {\it continue} to modulate the gain, overriding the setting you make here.  In general it's probably not  wise to modulate the Gain knob.
\end{itemize}
...
\paragraph{Loading and Clearing Just the Primary Patch}  Obviously you could load a new primary patch or clear it by choosing {\it Load Patch...} or {\it New Patch}: but this will clear out the subpatches as well.  There are times when you'd just like to replace the primary patch but keep the existing subpatches around.

If you choose {\it New Primary Patch}, you will clear the primary patch but not the subpatches.  You'll be asked if you wanted to optionally {\it demote} the primary patch, that is, to make it a subpatch rather than eliminate it. 

Similarly, if you choose {\it Load Primary Patch...}, you will load a new primary patch, replacing the original, but will keep the subpatches.  You will likewise be asked if you would like to optionally {\it demote} the primary patch just as before.

\paragraph{Strategy}  I am an old-school synthesizer guy: I tend to use my DAW to automate all my hardware synthesizers, then mix them in analog and record the final result.  I will use Flow's multitmbral patches to follow a similar strategy as follows.  I lay down each track on a different MIDI channel, targeting a different subpatch, and listen to all of them together.  I need more polyphony than my computer can provide, I will (temporarily) reduce the number of partials.  Then I will either play the final result directly into the DAW to record it (if I didn't have to reduce the number of partials), or increase the partials again and record each patch separately and mix the final result in the DAW.
...
Draw also allows you to load {\it single-cycle waveforms}, that is, sound files which start and end at zero and represent a single cycle of a sound wave.  You can do this by choosing {\it Load Wave...} from the {\it Menu} button.  Wave files must be mono, and cannot be very long (presently at most 2048 samples).  After loading, Flow converts them (naturally) into harmonics.
...
There are currently four special modules: {\bf In}, {\bf Out}, {\bf Choice}, {\bf Note}, and {\bf Macro}.  The {\bf Out} module is critical in every patch, and is discussed below in Section~\ref{outmodule}.  {\bf In} and {\bf Macro} enable {\it macros}\.---\,and {\bf Out} helps out here as well\,---\,as discussed below in Sectino~\ref{creatingmacros}.
...
\paragraph{Note} This module doesn't process sound at all. Instead, it lets you leave a note or comment.  You can change the width of the Note panel with two arrow-like buttons at the bottom.   You can have as many Note modules as you like; they'll each save their comments independently as part of your patch.
