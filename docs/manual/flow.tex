...
Glitches often settle down after running Flow a minute or so.  But if not, how should we tamp them down?  By playing with a few parameters.\footnote{One major source of glitches is Java's garbage collection, which takes a long time to do.  Java has a new low-latency garbage collector called the {\bf Z Collector}.  With recent versions of Java (20 say), you can use the Z collector to dramatically reduce the glitches by running Flow like this:\quad{\tt java -jar flow.jar -XX:+UseZGC -XX:MaxGCPauseMillis=1}\quad Flow's Mac app automatically does this.}  If you select {\bf Tuning Parameters} from the {\bf Options} menu, you can at present change six parameters.  Pressing {\it Okay} or {\it Reset} (which resets to defaults) will change the parameters for the {\it next} time you run {\name}, not the current session.   
...
\paragraph{Latch}  This is a mechanism to enable an initial impulse to flow through to output, but then immediately latch to some other input to flow to the output thereafter.  Its primary use is to start with a basic impulse sound, then recursively feed that impulse back through the Latch.  The {\it Initial} partials are flowed to the output when a {\it Trigger} occurs; otherwise the {\it Later} partials are flowed to the output.

You select from among four different Initial partials sources using {\it Sel Initial}.  You also select from among four different Later partials sources using {\it Sel Partial}.  
...
The problem with modToFrequency is that it takes too long to reach a reasonable value: so modToFrequency(0.5) is only 11 Hz.  This is a waste of dial space.  So we make the dial much less sensitive to the exponential-ish effect by including {\tt makeVeryInsensitive(...)}. Now the two together yield 6567 Hz at 0.5.
...
\paragraph{Add Bounds} 
Some filters, amplifiers, and other Units {\it increase} amplitudes.  If you placed them in a cycle, they could increase those amplitudes without bound.  We want those amplitudes to stay reasonable, or at least not go to infinity, at which point they can cause infinite or NaN amplitude values when summed and break everything.  To prevent this, we'll add a call to {\tt boundAmplitudes} at the very end of {\tt go()} to keep the amplitudes reasonable.  This is not very expensive but it is an O(n) call, so we'd like to only include it if we know the Unit we're building could do this.  As it so happens the one we're building here {\it cannot} increase amplitudes (it only decreases them to 0!), but let's include it just as an example.
...
\begin{verbatim}
    public void go()
          ...
          constrain();
          boundAmplitudes();        // as mentioned, not useful in this example...
          }
\end{verbatim}
}
