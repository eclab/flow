...
public class Input
{
	...
	void processPitchBend(ShortMessage sm)
	{
		...
		synchronized (lock)
			{
				int lin = (int)Math.ceil(rawBend / 8191.0 * getBendOctave());
				for (Sound sound : notesOn)
					{
						...
						if (c == CHANNEL_OMNI || c == sm.getChannel() || (isMPEChannel(c) && sm.getChannel() == getMPEGlobalChannel()))
							{
								if (microTuning != null) {
									int baseNote = sound.getMIDINote() + lin;
									if(baseNote > 127) { // Trying to bend past last note
										baseNote = 127;
									}
									double ratio = microTuning.getLerpRatio(sound.getMIDINote(), sound.getMIDINote() + lin, lin/getBendOctave());
									sound.setBend(ratio);
